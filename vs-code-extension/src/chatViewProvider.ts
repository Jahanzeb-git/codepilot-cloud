...
import { SessionSummary, AgentYamlSnapshot, ToolPermission, UpdateSettingsPayload, MAX_TOKENS_BY_MODEL } from './webview/chat/types';
...
async function readAgentYamlSnapshot(extensionUri: vscode.Uri): Promise<AgentYamlSnapshot | null> {
    try {
        const agentYamlPath = vscode.Uri.joinPath(extensionUri, 'dist', 'agent.yaml').fsPath;
        const raw = await fs.readFile(agentYamlPath, 'utf-8');
...
export class ChatViewProvider implements vscode.WebviewViewProvider {
    ...
    public async showSettingsPanel(): Promise<void> {
        ...
        const snapshot = await readAgentYamlSnapshot(this.extensionUri);
    ...
    private async handleAgentEvent(event: AgentEvent): Promise<void> {
        if (event.type === 'tool_call') {
            const tc = event as { type: string; tool?: string; args?: { session_id?: string } };
            if (tc.tool === 'execute') {
                const targetSession = tc.args?.session_id || 'main';
                TerminalManager.getOrCreate(this.extensionUri).revealTabOnFirstExecute(targetSession);
            }
        }
            ...
            <body class="settings-panel-body">
                ...
                <script nonce="${nonce}">
                    (function() {
                        ...
                        const MAX_TOKENS_BY_MODEL = ${JSON.stringify(MAX_TOKENS_BY_MODEL)};
