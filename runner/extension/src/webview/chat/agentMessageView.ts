...
export class AgentMessageView {
    ...
    private ensureThinkingEl(): void {
        ...
        header.innerHTML = '<span class="thinking-icon">▼</span> <span class="thinking-label">Thinking process</span>';

        const content = document.createElement('div');
        content.className = 'thinking-content';

        this.thinkingEl.appendChild(header);
        this.thinkingEl.appendChild(content);

        header.addEventListener('click', () => {
            this.thinkingEl!.classList.toggle('collapsed');
            const icon = header.querySelector('.thinking-icon')!;
            if (this.thinkingEl!.classList.contains('collapsed')) {
                icon.textContent = '▶';
            } else {
                icon.textContent = '▼';
            }
    ...
    private render(): void {
        let bodyHtml = md.render(this.rawText);
        this.bodyEl.innerHTML = bodyHtml;
        if (this.thinkingEl) {
            const content = this.thinkingEl.querySelector('.thinking-content')!;
            content.textContent = this.thinkingText;
            if (!this.isThinking) {
                this.thinkingEl.classList.add('collapsed');
                const icon = this.thinkingEl.querySelector('.thinking-icon')!;
                icon.textContent = '▶';
            }
    ...
    public finish(): void {
        ...
        if (this.toolCalls.size > 0) {
            ...
            this.workingHeaderText.textContent = `${actionsCount} ${actionLabel} taken (${totalSecs}s)`;
        } else {
    ...
    public appendError(message: string): void {
        this.finish(); // Stop timer and collapse the Working UI
        const el = document.createElement('div');
