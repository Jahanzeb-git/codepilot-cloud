...
export function renderSessionHistory(chatContainer: HTMLElement, messages: TranscriptMessage[]): void {
    ...
    for (const msg of real) {
        ...
        if (msg.role === 'assistant') {
            ...
            const prose = msg.content.replace(FENCED_BLOCK_RE, '').trim();
            if (!prose && toolNames.length === 0) continue;

            const view = new AgentMessageView(chatContainer);
            if (prose) {
                view.pushChunk(prose);
            }
